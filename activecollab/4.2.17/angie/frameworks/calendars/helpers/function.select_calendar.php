  ...
  function smarty_function_select_calendar($params, &$smarty) {
    ...
    $value = array_var($params, 'value', true, null);
	  ...
	  if (Calendars::canAdd($user)) {
		  ...
		  if (!$groups) {
			  $message = HTML::openTag('strong', null, lang("You don't have enough privilege to add events on any of these available calendars."));
			  $result .= HTML::openTag('p', array('class' => 'warning'), $message);
		  } // if
	  } // if
