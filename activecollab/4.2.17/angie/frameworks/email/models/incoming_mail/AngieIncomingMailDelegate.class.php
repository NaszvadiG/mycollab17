<?php
  ...
  class AngieIncomingMailDelegate extends AngieDelegate {
     ...
     * @return IncomingMail
     */
    function createPendingEmail(MailboxManagerEmail &$email, &$mailbox = null) {
      ...
      //if is reply then strip neccessery lines else use full body
      if($incoming_mail->isReplyToNotification()) {
        $mail_body = $body_procesor->extractReply();
      } else {
        $mail_body = $body_procesor->extractBody();
      } //if
      ...
      $sender = $email->getAddress('reply_to');
