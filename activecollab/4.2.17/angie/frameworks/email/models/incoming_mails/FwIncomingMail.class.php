<?php
  ...
  abstract class FwIncomingMail extends BaseIncomingMail implements IRoutingContext,IReadOnly {
    ...
    /**
     * Return array containing from and to value if auto forward used
     *
     * @return array [from, to]
     */
    function getForwardedInfoFromHeader() {
      $raw_headers = explode("\n", $this->getHeaders());

      if (is_foreachable($raw_headers)) {
        foreach ($raw_headers as $k => $raw_header) {
          $raw_header = trim($raw_header);

          //X-Forwarded-For (gaxoman@gmail.com igorg+m2p-31a3356@mail.manageprojects.com) - gmail
          if (preg_match('/^(X-Forwarded-For[^:]*): (.*)/is', $raw_header, $results)) {
            $header_name = trim(isset($results[1]) ? $results[1] : null);
            $header_value = trim(isset($results[2]) ? $results[2] : null);
            if ($header_name) {
          ...
          //X-Yahoo-Forwarded: from gaxoman@yahoo.com to igorg+m2p-31a3356@mail.manageprojects.com - yahoo
          if (preg_match('/^(X-Yahoo-Forwarded[^:]*): (.*)/is', $raw_header, $results)) {
            $header_name = trim(isset($results[1]) ? $results[1] : null);
            $header_value = trim(isset($results[2]) ? $results[2] : null);
            if ($header_name) {
              $tmp = explode(" ", $header_value);

              if($tmp[3] && (boolean) strpos_utf($tmp[3], '@')) {
                return array($tmp[1], $tmp[3]);
              } else {
                //we have forwarded_to value in newline
                $forwarded_to = trim($raw_headers[$k+1]); //first next
                if((boolean) strpos_utf($forwarded_to, '@')) {
                  return array($tmp[1], $forwarded_to);
                } //if
              } //if
            } // if
          } // if
