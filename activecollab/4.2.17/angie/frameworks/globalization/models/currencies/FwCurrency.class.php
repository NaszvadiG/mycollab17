<?php
  ...
  class FwCurrency extends BaseCurrency implements IRoutingContext {
    ...
    const CURRENCY_CODE_POSITION_LEFT = 'left';
    const CURRENCY_CODE_POSITION_RIGHT = 'right';
    ...
    function describe(IUser $user, $detailed = false, $for_interface = false) {
      ...
      $result['code_position'] = $this->getCodePosition();
    ...
    function validate(ValidationErrors &$errors) {
      ...
      if($this->validatePresenceOf('code')) {
        ...
        if(trim(strlen($this->getCode())) > 3) {
          $errors->addError(lang('Currency code needs to have 3 letters max'));
        } //if
      } else {
