    ...
    if($interface == AngieApplication::INTERFACE_DEFAULT) {
      ...
      foreach($options as $option_name => $option_text) {
        ...
        if($option_name == Reminder::REMIND_SELECTED) {
          ...
          if ($object instanceof ISubscriptions) {
            $select_picker_params['users'] = $object->subscriptions()->getAvailableUsersForSelect($user);
          } // if

          $result .= '<div class="select_reminder_to_option_content" style="display: ' . ($selected ? 'block' : 'none') . '">' . smarty_function_select_user($select_picker_params, $smarty) . '</div>';
        } // if
