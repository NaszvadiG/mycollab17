<?php
  ...
  abstract class FwLabels extends BaseLabels {
    ...
    static function findByType($type, $as_uppercase = true) {
      $cached_values = AngieApplication::cache()->get(array('labels', $type));
      if ($cached_values) {
      ...
      AngieApplication::cache()->set(array('labels', $type), $labels);
      return $labels;
    } // findByType
    ...
    static function getIdNameMap($type = null, $as_uppercase = true) {
      if ($type) {
        ...
        $result = AngieApplication::cache()->get(array('labels', 'id_name_map'), function() use ($as_uppercase) {
          $id_name_map = array();
          $rows = FwLabels::find();
    ...
    static function getIdDetailsMap($type, $always_uppercase = true) {
      $cache_key = array('labels', 'id_details_map', $type);
      ...
      foreach($rows as $row) {
        ...
        $result[(integer) $row['id']] = array(
          'name' => $always_uppercase ? strtoupper($row['name']) : $row['name'],
