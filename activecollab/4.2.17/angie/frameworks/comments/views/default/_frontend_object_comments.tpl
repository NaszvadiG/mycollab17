	      ...
	      <div class="frontend_object_comment_avatar">
		    ...
		    <div class="frontend_object_comment_content">
				  <div class="frontend_object_comment_meta">
				    {if $user instanceof User}
				      {lang author_name=$comment->getCreatedBy()->getDisplayName(true) author_url=$comment->getCreatedBy()->getViewUrl() on=$comment->getCreatedOn()->formatForUser()}<a class="comment_author" href=":author_url">:author_name</a> on <span class="comment_date">:on</span>{/lang}
				    {else}
				      {lang author_name=$comment->getCreatedBy()->getDisplayName(true) on=$comment->getCreatedOn()->formatForUser()}<span class="comment_author" >:author_name</span> on <span class="comment_date">:on</span>{/lang}
				    {/if}
				  </div>
