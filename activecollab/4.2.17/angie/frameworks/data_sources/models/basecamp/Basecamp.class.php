  ...
  class Basecamp extends BasecampSource implements IDescribe {
    ...
    */
    private function importUser($bc_user) {
      ...
      $bc_user_email = $this->makeValidEmail($bc_user_email);
      ...
      //new user
        if(php_sapi_name() == 'cli') {
          echo "Importing user: ". $bc_user->email_address . "...\r\n";
        } //if
     ...
     * @param $check
     */
    public function importProject($bc_project_id, $check = true) {
      ...
      if(!$project instanceof Project || $check === false) {
        //isn't already imported

        if(php_sapi_name() == 'cli') {
          echo "Fetching BC Project with Id " . $bc_project_id . " from Basecamp...\r\n";
        } //if
    ...
    private function addToDoListToProjectByUrl(Project $project, $to_do_list_url) {
      ...
      if(php_sapi_name() == 'cli') {
        echo "Adding ToDo List '" . $this->maxLength($to_do_list->name). "' to Project...\r\n";
      } //if
      ...
      switch($this->getImportSettings()) {
        ...
        case Basecamp::IMPORT_SETTINGS_TODO_LIST_AS_TASK_CATEGORY:
          $cat_name = $this->maxLength($to_do_list->name, 90);
          $same_category = Categories::find(array(
            'conditions' => array('parent_type = ? AND parent_id = ? AND type = ? AND name = ?', get_class($project), $project->getId(), 'TaskCategory', $cat_name),
            'one' => true
          ));
          if($same_category instanceof TaskCategory) {
            $object = $same_category;
          } else {
            $object = new TaskCategory();
            $object->setParent($project);
            $object->setName($cat_name);
            $object->save();
          } //if
          ...
          if(is_foreachable($to_do_list->comments)) {
            ...
            $body = HTML::cleanUpHtml(trim($to_do_list->description));
            $body = $body ? $body : lang('Body not provided or removed');
    ...
    public function addToDoToObjectByUrl(ApplicationObject $parent, $todo_url) {
      ...
      if(php_sapi_name() == 'cli') {
        echo "Adding ToDo '" . $this->maxLength($todo->content). "' to '" . $parent->getVerboseType() . ":". $parent->getName() . "'...\r\n";
      } //if
    ...
    private function completeObject(ApplicationObject $object, IUser $by, DateTimeValue $on = null, $gag_modification_log = false) {
      if(php_sapi_name() == 'cli') {
        echo "Completing Object: '". $object->getVerboseType() . ":" . $object->getName() . "'...\r\n";
      } //if
    ...
    public function addDiscussionsToProjects(Project $project, $bc_project) {
      ...
      if(is_foreachable($discussions)) {
        ...
        foreach($discussions as $discussion_block) {
          ...
          if(php_sapi_name() == 'cli') {
            echo "Adding Discussion:'" . $this->maxLength($discussion_data->subject) . "' to Project...\r\n";
          } //if
          $new_discussion = new Discussion();

          $body = HTML::cleanUpHtml(trim($discussion_data->content));
          $body = $body ? $body : lang('Body not provided or removed');
    ...
    public function addTextDocumentsToProjects(Project $project, $bc_project) {
      ...
      if(is_foreachable($text_documents)) {
        foreach($text_documents as $document_block) {
          ...
          if(php_sapi_name() == 'cli') {
            echo "Adding Text Document '" . $this->maxLength($text_document_data->title) . "' to Project...\r\n";
          } //if
    ...
    private function addAttachments(IAttachments $parent, $bc_attachments_block) {
      ...
      if(is_foreachable($bc_attachments_block)) {
        foreach($bc_attachments_block as $bc_attachment) {
          ...
          if(php_sapi_name() == 'cli') {
            echo "Adding Attachment: '" . $bc_attachment->name . " to '". $parent->getName() ."' ...\r\n";
          } //if
    ...
    private function addComments(IComments $parent, $bc_comments) {
      ...
      if(is_foreachable($bc_comments)) {
        foreach($bc_comments as $bc_comment) {
          ...
          if(php_sapi_name() == 'cli') {
            echo "Adding Comment to '". $parent->getName() ."' ...\r\n";
          } //if
          ...
          if(trim($bc_comment->content) == '') {
            $bc_comment->content = '-';
          } //if

          //don't send email notifications for new comments
          $additional_params['notify_subscribers'] = false;
    ...
    */
    ...
    /**
     * Make email valid if it is not already. dummy email
     * @param $email
     * @return string
     */
    public function makeValidEmail($email) {
      if(!is_valid_email($email)) {
        list($username, $domain) = explode('@', $email);
        $email = $username . '_at_' . $domain . '@thisemailwasinvalid.com';
      } //if
      return $email;
    } //makeValidEmail
    ...
    public function getUsersForImport() {
      ...
      if(is_foreachable($all_bc_users)) {
        foreach($all_bc_users as $bc_user) {
          $bc_user->email_address = $this->makeValidEmail($bc_user->email_address);
