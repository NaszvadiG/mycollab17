<?php
  ...
  class ISubscriptionsImplementation {
     ...
     * @param $user_ids
     */
    function subscribeAllUsers($user_ids) {
      if(is_foreachable($user_ids)) {
        $users = Users::findByIds($user_ids);
        $parent_type = get_class($this->object);
        $parent_id = $this->object->getId();
        foreach($users as $user) {
          if(!$this->object->subscriptions()->isSubscribed($user)){
            $to_insert[] = DB::prepare('(?, ?, ?, ?, ?, UTC_TIMESTAMP(), ?)', $parent_type, $parent_id, $user->getId(), $user->getDisplayName(), $user->getEmail(), $this->getSubscriptionCode());
          } //if
        } //foreach
        if(count($to_insert)) {
          DB::execute('INSERT INTO ' . TABLE_PREFIX . 'subscriptions (parent_type, parent_id, user_id, user_name, user_email, subscribed_on, code) VALUES ' . implode(', ', $to_insert));
    ...
    function canSubscribe(User $user) {
      if($this->object->getState() < STATE_VISIBLE) {
        return $this->object->subscriptions()->isSubscribed($user);
      } // if
     ...
     * @return string
     */
    function getSubscribeAllUrl() {
      return Router::assemble($this->object->getRoutingContext() . '_subscribe_all', $this->object->getRoutingContextParams());
    } // getSubscriptionsUrl
