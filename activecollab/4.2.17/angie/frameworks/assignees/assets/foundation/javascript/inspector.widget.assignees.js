...
App.Inspector.Widgets.Assignees = function (object, client_interface) {
  ...
  if (object.assignee) {
    ...
    trigger.append('<span class="assignees_responsible" assignee_id="' + object.assignee.id + '">' + App.clean(object.assignee.display_name) + '</span>');
    ...
    trigger.append('<span class="assignees_responsible" assignee_id="0">' + App.lang('No one is<br />responsible') + '</span>');
    trigger.append('<span class="assignees_other_assignees">' + App.lang(':num other assignee(s)', { 'num' : object.other_assignees.length }) + '</span>');
  } else {
    trigger.append('<span class="assignees_widget_avatar"><img src="' + App.Wireframe.Utils.imageUrl('icons/32x32/assignees.png', 'assignees') + '" alt="" /></span>');
    trigger.append('<span class="assignees_responsible" assignee_id="0">' + App.lang('No one is<br />assigned') + '</span>');
  } // if
