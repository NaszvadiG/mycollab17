<?php
  ...
  function smarty_function_select_assignees($params, &$smarty) {
    ...
    $all_assignee_ids = $object->assignees()->getAllAssigneeIds();
    if (is_foreachable($all_assignee_ids)) {;
      // awful workaround START
      // required because we should merge visible users with already assigned users that MAY NOT BE VISIBLE in case that object doesn't have assignees (clients in aC with that permission turned off)
      $user_ids_for_select = $all_assignee_ids;
      $available_users = $object->assignees()->getAvailableUsers($user);
      if (is_foreachable($available_users)) {
        foreach ($available_users as $available_user) {
          if (!in_array($available_user->getId(), $user_ids_for_select)) {
            $user_ids_for_select[] = $available_user->getId();
          } // if
        } // foreach
      } // if
      // awful workaround END

      $exclude_ids_sql = $exclude_ids && is_foreachable($exclude_ids) ? DB::prepare("AND id NOT IN (?)", $exclude_ids) : "";

      $users = Users::getForSelectByConditions(DB::prepare("id IN (?) {$exclude_ids_sql} AND state >= ?", $user_ids_for_select, STATE_VISIBLE));
    } else {
      $users = $object->assignees()->getAvailableUsersForSelect($user, $exclude_ids);
    } // if
    ...
    if ($choose_subscribers && $object instanceof ISubscriptions) {
      if (array_key_exists('subscribers', $params)) {
        ...
        $subscriber_ids = $object->isLoaded() ? $object->getSubscriberIds() : null;
      } // if
    } else {
      $subscriber_ids = null;
    } // if
