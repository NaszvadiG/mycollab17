...
<div class="shared_notebook_wrapper">
  ...
  <div class="shared_notebook">
    ...
    {if $active_shared_notebook_page->getBody()}
      {$active_shared_notebook_page->getBody()|rich_text:frontend nofilter}
    {else}
      <p class="empty_page">{lang}Content not provided{/lang}</p>
    {/if}

    {if $active_shared_notebook_page_attachments}
      <div class="body_attachment">
        <strong>Attachments</strong>
        <ul>
        {foreach $active_shared_notebook_page_attachments as $active_shared_notebook_page_attachment}
          <li><a href="{$active_shared_notebook_page_attachment->getPublicViewUrl(true)}"><img src="{$active_shared_notebook_page_attachment->preview()->getSmallIconUrl()}"><span class="label">{$active_shared_notebook_page_attachment->getName()}</span></a><span class="size">{$active_shared_notebook_page_attachment->getSize()|format_file_size}</span></li>
        {/foreach}
        </ul>
      </div>
    {/if}
