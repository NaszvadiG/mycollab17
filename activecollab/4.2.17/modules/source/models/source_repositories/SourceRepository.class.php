    ...
    function update($logs, $branch) {
      ...
      if (is_foreachable($logs)) {
        ...
        // commit insert
          ...
          DB::execute("INSERT INTO ".TABLE_PREFIX."source_commits
                    (`name`,`type`,`revision_number`,`repository_id`,`message_title`,`message_body`,`authored_on`,`authored_by_name`,`authored_by_email`,`commited_on`,`commited_by_name`,`commited_by_email`,`branch_name`)
                    VALUES
                    (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    $data['commit']['name'], $data['commit']['type'], $data['commit']['revision_number'], $data['commit']['repository_id'], $data['commit']['message_title'], $data['commit']['message_body'], $authored_on, $data['commit']['authored_by_name'], $data['commit']['authored_by_email'], $commited_on, $data['commit']['commited_by_name'], $data['commit']['commited_by_email'], $branch);
          
          $last_inserted_commit = DB::lastInsertId();
          // search index insert
          ...
          if (is_foreachable($paths)) {
            ...
            foreach ($paths as $i => $path_log) {
              $path_insert[] = DB::prepare("(?, ?, ?)", $last_inserted_commit, $path_log['path'], $path_log['action']);
