  ...
  class RepositoryController extends ProjectController {
    ...
    function __before() {
      ...
      if($this->project_object_repository instanceof ProjectSourceRepository) {
        ...
        // active commit
        $revision = $this->request->get('r');
        $this->active_revision = is_null($revision) ? null : (integer) $revision;
    ...
    function history() {
      if($this->project_object_repository->canView($this->logged_user)) {
        ...
        if ($filter_by_author === null) {
          $conditions = $this->active_branch ? array('repository_id = ? AND branch_name = ?',$this->active_repository->getId(), $this->active_branch) : array('repository_id = ?', $this->active_repository->getId());

          $total_commits = SourceCommits::count($conditions);
          ...
          $conditions = $this->active_branch ?
                        array('repository_id = ? AND commited_by_name = ? AND branch_name = ?',$this->active_repository->getId(), $filter_by_author, $this->active_branch) :
                        array('repository_id = ? AND commited_by_name = ?',$this->active_repository->getId(), $filter_by_author);
        ...
        if (is_foreachable($commits_object)) {
          foreach ($commits_object as $commit) {
        ...
        $commits = group_by_date($commits, null, 'getCommitedOn', false);
    ...
    function commit() {
      ...
      if (RepositoryEngine::pathInFolder($this->active_file)) {
	      ...
	      if (!is_array($diff)) {
            ...
            if (strlen($serialized_diff) < $max_packet_size && !$this->active_commit->isNew()) {
