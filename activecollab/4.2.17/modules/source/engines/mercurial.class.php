   ...
   */
    function getLogs($revision_to, $revision_from) {
      ...
      foreach ($logs as $key=>$log_entry) {
        ...
        $email_for_analyze = is_valid_email($log_entry['author']['attr']['email']) ? $log_entry['author']['attr']['email'] : "nobody@site.com";
        $message_body = SourceCommit::analyze_message($log_entry['msg']['value'], $log_entry['author']['value'], $email_for_analyze, $log_entry['attr']['revision'], $this->active_repository, $this->active_branch);

        $message_title = first(explode("\n", $message_body));

        $commiter = $log_entry['author']['value'];
        $created_by_email = is_valid_email($log_entry['author']['attr']['email']) ? $log_entry['author']['attr']['email'] : "";
