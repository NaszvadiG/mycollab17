        ...
        for ($commit_id = $revision_from; $commit_id <= $revision_to; $commit_id++) {
          ...
          if (is_foreachable($parents)) {
            ...
            foreach ($dir_lists as $path=>$tree_name) {
              $paths[$k]['path'] = $path; // paths can contain file names with characters that can break the query
              $paths[$k]['action'] = 'A';
              $k++;
            } // foreach
          } //if
          ...
          $committer = $commit_object->committer->name;

          $email_for_analyze = is_valid_email($commit_object->author->email) ? $commit_object->author->email : "nobody@site.com";
          $message_body = SourceCommit::analyze_message($commit_object->summary."\n".$commit_object->detail, $commit_object->committer->name, $email_for_analyze, $key+1, $this->active_repository, $this->active_branch);

          $message_title = first(explode("\n", $message_body));

          $updated_by_email = is_valid_email($commit_object->author->email) ? $commit_object->author->email : "";
          $created_by_email = is_valid_email($commit_object->committer->email) ? $commit_object->committer->email : "";
