  ...
  class Invoice extends InvoiceObject implements IState, IPayments, IConfigContext, IActivityLogs, IHistory, IInspector, IInvoice {
        ...
        try {
          ...
          if(!$payment instanceof CustomPayment) {
            if($date instanceof DateValue) {
              $date = $date->advance(get_system_gmt_offset(), false);
            } //if
          } //if
    ...
    function describeForApi(IUser $user, $detailed = false) {
      $result = array(
        ...
        'issued_on' => $this->getIssuedOn() ? $this->getIssuedOn()->toMySQL() : null,
        'due_on' => $this->getDueOn() ? $this->getDueOn()->toMySQL() : null,
        'items' => array(),
    ...
    function validate(ValidationErrors &$errors) {
      ...
      if($this->validatePresenceOf('varchar_field_1')) {
        $additional_condition = !$this->isNew() ? "AND id != '{$this->getId()}'" : "";
        if (DB::executeFirstCell("SELECT COUNT(id) FROM " . TABLE_PREFIX . "invoice_objects WHERE type = 'Invoice' AND state > ? AND varchar_field_1 = ? {$additional_condition}", STATE_DELETED, $this->getVarcharField1())) {
          $errors->addError(lang('Invoice #:invoice_id needs to be unique', array('invoice_id' => $this->getVarcharField1())), 'number');
        } // if
      } // if
    } // validate
  }