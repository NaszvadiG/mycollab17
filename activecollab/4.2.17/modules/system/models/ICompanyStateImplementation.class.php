  ...
  class ICompanyStateImplementation extends IStateImplementation {
    ...
    function delete() {
      try {
        ...
        $company_projects_count = DB::executeFirstCell("SELECT COUNT(id) FROM " . TABLE_PREFIX . "projects WHERE company_id = ?", $this->object->getId());
        if ($company_projects_count) {
          DB::execute("UPDATE " . TABLE_PREFIX . "projects SET company_id = ? WHERE company_id = ?", Companies::findOwnerCompany()->getId(), $this->object->getId());
