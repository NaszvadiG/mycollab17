  ...
  class Attachments extends FwAttachments {
    ...
    static function exportToFileByProject(Project $project, $output_file, &$parents_map, $changes_since, $for_full_export = false) {
      ...
      if(is_foreachable($parents_map)) {
        ...
        $attachment = DB::execute("SELECT id, type, parent_type, parent_id, state, name, mime_type, size, location, md5, created_on, created_by_id, created_by_name, created_by_email FROM $attachments_table WHERE ($conditions) AND state >= ? $additional_condition", $min_state);
