<?php
  ...
  class MailToProjectInterceptor extends IncomingMailInterceptor {
    ...
    function match(IncomingMail $incoming_mail) {
      ...
      //in case that we have forwarded part in header with m2p address
      list($forwarded_from, $forwarded_to) = $incoming_mail->getForwardedInfoFromHeader();
      if($forwarded_to) {
        if((boolean) strpos_utf($forwarded_to, self::M2P_DELIMITER)) {
          return true;
        } //if
      } //if
    ...
    function execute(IncomingMail $incoming_mail, $additional_params = null) {
      ...
      //maybe is forwarded to m2p address
      if(!$email) {
        list($forwarded_from, $email) = $incoming_mail->getForwardedInfoFromHeader();
      } //if
