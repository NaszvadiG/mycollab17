<?php
  ...
  class Users extends FwUsers {
    ...
    static function exportToFile() {
      $result = array();
      $companies = Companies::findActiveAndArchived();
      foreach ($companies as $k => $company) {
        $result[$k]['name'] = $company->getName();
        $result[$k]['note'] = $company->getNote();
        $result[$k]['created_on'] = $company->getCreatedOn() ? $company->getCreatedOn()->getTimestamp() : null;
        $result[$k]['updated_on'] = $company->getUpdatedOn() ? $company->getUpdatedOn()->getTimestamp() : null;
        $result[$k]['created_by_id'] = (integer) $company->getCreatedById();
        $result[$k]['updated_by_id'] = (integer) $company->getUpdatedById();
        $result[$k]['is_archived'] = (boolean) $company->getState() == STATE_ARCHIVED;
        $result[$k]['icon'] = base64_encode(file_get_contents($company->avatar()->getUrl(IProjectAvatarImplementation::SIZE_PHOTO)));
        $result[$k]['office_address'] = $company->getConfigValue('office_address');
        $result[$k]['office_phone'] = $company->getConfigValue('office_phone');
        $result[$k]['office_fax'] = $company->getConfigValue('office_fax');
        $result[$k]['office_homepage'] = $company->getConfigValue('office_homepage');
        $result[$k]['users'] = array();

        $users = Users::findByCompany($company, array(), STATE_ARCHIVED);
        if (is_foreachable($users)) {
          foreach ($users as $ku => $user) {
            $result[$k]['users'][$ku]['id'] = $user->getId();
            $result[$k]['users'][$ku]['type'] = $user->getType();
            $result[$k]['users'][$ku]['email'] = $user->getEmail();
            $result[$k]['users'][$ku]['first_name'] = $user->getFirstName();
            $result[$k]['users'][$ku]['last_name'] = $user->getLastName();
            $result[$k]['users'][$ku]['name'] = $user->getName();
            $result[$k]['users'][$ku]['display_name'] = $user->getDisplayName();
            $result[$k]['users'][$ku]['short_display_name'] = $user->getDisplayName(true);
            $result[$k]['users'][$ku]['avatar_full_size'] = base64_encode(file_get_contents($user->avatar()->getUrl(IUserAvatarImplementation::SIZE_PHOTO)));
            $result[$k]['users'][$ku]['company_id'] = $user->getCompanyId();
            $result[$k]['users'][$ku]['created_by_id'] = $user->getCreatedById();
            $result[$k]['users'][$ku]['created_on'] = $user->getCreatedOn() instanceof DateTimeValue ? $user->getCreatedOn()->getTimestamp() : null;
            $result[$k]['users'][$ku]['is_administrator'] = $user->isAdministrator();
            $result[$k]['users'][$ku]['is_archived'] = $user->isArchived();
            $result[$k]['users'][$ku]['is_people_manager'] = $user->isPeopleManager();
            $result[$k]['users'][$ku]['is_project_manager'] = $user->isProjectManager();
            $result[$k]['users'][$ku]['last_activity_on'] = $user->getLastActivityOn() instanceof DateTimeValue ? $user->getLastActivityOn()->getTimestamp() : null;
            $result[$k]['users'][$ku]['state'] = $user->getState();
            $result[$k]['users'][$ku]['updated_by_id'] = $user->getUpdatedById();
            $result[$k]['users'][$ku]['updated_on'] = $user->getUpdatedOn() instanceof DateTimeValue ? $user->getUpdatedOn()->getTimestamp() : null;
            $result[$k]['users'][$ku]['permissions'] = $user->getSystemPermissions();
          } // foreach
        } // if
      } // foreach

      $output_path = WORK_PATH . "/people.json";
      file_put_contents($output_path, "[" . JSON::encode($result) . "]");
    ...
    static function findForObjectsList(User $user, $min_state = STATE_ARCHIVED) {
      $users = DB::execute("SELECT id, state, first_name, last_name, email, company_id, UNIX_TIMESTAMP(updated_on) AS updated FROM " . TABLE_PREFIX . "users WHERE company_id IN (?) AND id IN (?) AND state >= ? ORDER BY CONCAT(first_name, last_name, email)", $user->visibleCompanyIds(), $user->visibleUserIds(), $min_state);

      $users_map = array();
      if ($users) {
        $view_url_pattern = Router::assemble('people_company_user', array("company_id" => "--CID--", "user_id" => "--ID--"));

        foreach ($users as $v) {
          if ($v['first_name'] || $v['last_name']) {
            $name = trim($v['first_name'] . ' ' . $v['last_name']);
          } else {
            $name = substr($v['email'], 0, strpos($v['email'], '@'));
          } // if

          $avatar_path = ENVIRONMENT_PATH . '/' . PUBLIC_FOLDER_NAME . '/avatars/' . $v['id'] . '.16x16.png';
          $avatar_url = ROOT_URL . '/avatars/' . (is_file($avatar_path) ? $v['id'] : 'default') . '.16x16.png?time=' . $v['updated'];

          $users_map[] = array(
            'id' => (integer) $v['id'],
            'is_archived' => (integer) ($v['state'] == STATE_ARCHIVED),
            'name' => $name,
            'avatar' => array(
              'small' => $avatar_url,
              'photo' => ''
            ...
            'display_name' => $name,
            'email' => $v['email'],
            'company_id' => (integer) $v['company_id'],
            'permalink' => str_replace(array("--CID--", "--ID--"), array($v['company_id'], $v['id']), $view_url_pattern)
          );
        } // foreach
      } // if
