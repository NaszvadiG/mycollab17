  ...
  class Projects extends BaseProjects {
    ...
    private static function _findByUser(User $user, $all_for_admins_and_pms = false, $additional_conditions = null, $order_by = null) {
      ...
      if($all_for_admins_and_pms && $user->isProjectManager()) {
        $additional_conditions_sql = $additional_conditions ? "WHERE {$additional_conditions}" : "";
        return Projects::findBySQL("SELECT * FROM $projects_table $additional_conditions_sql ORDER BY $order_by");
      } else {
        $additional_conditions_sql = $additional_conditions ? "AND {$additional_conditions}" : "";
        return Projects::findBySQL("SELECT $projects_table.* FROM $projects_table LEFT JOIN $project_users_table ON $project_users_table.project_id = $projects_table.id WHERE $project_users_table.user_id = ? $additional_conditions_sql ORDER BY $order_by", $user->getId());
      } // if
    } // _findByUser
