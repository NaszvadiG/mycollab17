<?php
  ...
  class Project extends BaseProject implements IRoutingContext, IConfigContext, IUsersContext, IComplete, ILabel, ICategoriesContext, ICategory, IHistory, IState, ISearchItem, IAvatar, ICanBeFavorite, ITracking, IInvoiceBasedOn, IActivityLogs, IObjectContext, ICustomFields, IAccessLog, ICalendarContext {
     ...
     * @param bool $for_full_export
     * @return array
     * @throws Error
     */
    function exportToFile(User $user, $output_file, $for_full_export = false) {
      ...
      foreach(array('asset' => 'AssetCategory', 'discussion' => 'DiscussionCategory', 'task' => 'TaskCategory') as $k => $type) {
        $categories = Categories::getIdNameMap($this, $type);

        if($categories && is_foreachable($categories)) {
          $project['categories'][$k] = $categories;
        } else {
          $project['categories'][$k] = array();
        } // if
      } // foreach
     ...
     * @throws Exception
     */
    function exportAsFile(User $user, $skip_files, $changes_since, $for_full_export = false) {
      $export_dir_path = WORK_PATH . '/' . ($for_full_export ? "projects/" : "") . $this->getSlug();
      ...
      $this->exportToFile($user, $export_dir_path . '/project.json', $for_full_export);

      $parents_map = array();

      $milestones_count = Milestones::exportToFileByProject($this, $user, $export_dir_path . '/milestones.json', $parents_map, $changes_since, $for_full_export);
      $tasks_count = Tasks::exportToFileByProject($this, $user, $export_dir_path . '/tasks.json', $parents_map, $changes_since, $for_full_export);
      $discussions_count = Discussions::exportToFileByProject($this, $user, $export_dir_path . '/discussions.json', $parents_map, $changes_since, $for_full_export);
      $assets_count = ProjectAssets::exportToFileByProject($this, $user, $export_dir_path . '/assets.json', $parents_map, $changes_since, null, $for_full_export);
      $notebooks_count = Notebooks::exportToFileByProject($this, $user, $export_dir_path . '/notebooks.json', $parents_map, $changes_since, $for_full_export);
      $notebook_pages_count = NotebookPages::exportToFileByProject($this, $user, $export_dir_path . '/notebook_pages.json', $parents_map, $changes_since, $for_full_export);
      $tracking_count = TrackingObjects::exportToFileByProject($this, $user, $export_dir_path . '/tracking.json', $parents_map, $changes_since, $for_full_export);
      $comments_count = Comments::exportToFileByProject($this, $export_dir_path . '/comments.json', $parents_map, $changes_since, $for_full_export);
      $subtasks_count = Subtasks::exportToFileByProject($this, $export_dir_path . '/subtasks.json', $parents_map, $changes_since, $for_full_export);
      $attachments_count = Attachments::exportToFileByProject($this, $export_dir_path . '/attachments.json', $parents_map, $changes_since, $for_full_export);

      // export
      if(!$skip_files) {
        Project::exportBinaryData($this, $user, $parents_map, $changes_since, $export_dir_path, $for_full_export);
      } // if

      if (!$for_full_export) { //  don't zip yet and don't send its output... (parent command will do it)
     ...
     * @throws Exception
     */
    static function exportBinaryData(Project $project, User $user, $parents_map, $changes_since, $export_dir_path, $for_full_export = false) {
      ...
      $files = ProjectAssets::findForExport($project, $user, $parents_map, $changes_since, array('File'), $for_full_export);
      ...
      if(is_foreachable($files) || is_foreachable($attachments)) {
        foreach(array_merge($files, $attachments) as $file) {
          if (!is_file(UPLOAD_PATH . '/' . $file['location'])) {
            continue;
          } // if
