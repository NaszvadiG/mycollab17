...
<div id="release_notes_wrapper"  style="display:none;height: 500px; overflow:auto;">
    <div id="warnings_content"></div>
    <div id="release_notes_content"></div>
</div>
<div class="button_holder" style="padding: 5px; display: none;">
  {submit class="do_upgrade"}Upgrade{/submit}
  {button class="cancel_upgrade"}Upgrade Later{/button}
</div>
...
<script type="text/javascript">
  ...
  var release_notes = {$release_notes|json nofilter};
  var upgrade_warnings = {$upgrade_warnings|json nofilter};
  ...
  var release_notes_wrapper = $('#release_notes_wrapper');
  var warnings_content = release_notes_wrapper.find('#warnings_content');
  var release_notes_content = release_notes_wrapper.find('#release_notes_content');

  $('button.do_upgrade').click(function() {
    $('div.button_holder').hide();
    release_notes_wrapper.slideUp(function() {
      wizard.slideDown();
    });
  });

  $('button.cancel_upgrade').click(function() {
    $('a.flyout_dialog_close_button').click();
  });

  if (release_notes) {
    $.each(release_notes, function(index, data) {
      $('<div class="release_notes_item"><h3>activeCollab '+data.version+'</h3><div>' + data.text + '</div></div>').appendTo(release_notes_content);
    });
  } // if

  if (upgrade_warnings) {
    warnings_content.show();
    $.each(upgrade_warnings, function(index, data) {
      $('<h3>activeCollab '+data.version+'</h3><div class="warnings_item">' + data.text + '</div>').appendTo(warnings_content);
    });
  } // if

  if (release_notes || upgrade_warnings) {
    release_notes_wrapper.show();
    $('div.button_holder').show();
  } // if
...
<style type="text/css">
  ...
  #release_notes_wrapper {
    display: none;
  }

  #release_notes_wrapper h3 {
    font-weight: bold;
  }
  ...
  #release_notes_content div.release_notes_item {
    margin-bottom: 20px;
  }

  #release_notes_content div.release_notes_item div {
    margin-left: 10px;
  }
